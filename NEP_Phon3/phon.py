...
def calculate_primitive_matrix(unit_cell, primitive_cell):
    ...
    计算原胞矩阵
    """
    return np.dot(np.linalg.inv(unit_cell), primitive_cell)
...
def nep_calculator(filename="../7.0-5.0-80.txt", use_dftd3=False):
    """
...
def calculate_forces_by_nep(structure):    
    """
    使用NEP计算结构的原子受力
    """
...
def calculate_force_constants_by_nep(structure, primitive_matrix, supercell_matrix, fmax=0.0001, distance=0.01):    
    """
    使用NEP计算力常数
    """
    structure.calc = nep_calculator()
    ...
    for phonopy_supercell in phonon.supercells_with_displacements:
        ...
        forces.append(calculate_forces_by_nep(structure_ase))
...
def calculate_gruneisen_parameters(unit_cell, primitive_matrix, supercell_matrix, kpoints_rel, mesh_dims=[20, 20, 20], scale_factor=0.01):
    """
    计算格林艾森参数
    """
    ...
    phonon = calculate_force_constants_by_nep(unit_cell, primitive_matrix, supercell_matrix)
    phonon_plus = calculate_force_constants_by_nep(unit_cell_plus, primitive_matrix, supercell_matrix)
    phonon_minus = calculate_force_constants_by_nep(unit_cell_minus, primitive_matrix, supercell_matrix)
